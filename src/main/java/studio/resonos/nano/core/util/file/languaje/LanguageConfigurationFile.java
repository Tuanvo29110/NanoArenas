package studio.resonos.nano.core.util.file.languaje;
...
import studio.resonos.nano.core.util.file.AbstractConfigurationFile;
...
public class LanguageConfigurationFile extends AbstractConfigurationFile {
    ...
    public LanguageConfigurationFile(JavaPlugin plugin, String name, boolean overwrite) {
        ...
        for (Lang locale : Lang.values()) {
            Bukkit.getConsoleSender().sendMessage(CC.translate("&eLoading language file for &a" + locale.name()));
            ...
            if (plugin.getResource(path) != null) {
                ...
                Bukkit.getConsoleSender().sendMessage(CC.translate("&eLanguage file for &a" + locale.name() + " &esuccessfully loaded"));
            }
        }
    }
    ...
    public void reload() {
        for (Map.Entry<Lang, YamlConfiguration> entry : getConfigurations().entrySet()) {
            Lang type = entry.getKey();
            YamlConfiguration configuration = entry.getValue();
            File file = new File(NanoArenas.get().getDataFolder(),
                    "languages/lang" + "_" + type.getAbbreviation() + ".yml");
            try {
